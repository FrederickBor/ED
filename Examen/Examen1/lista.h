...
public:
	...
	/**
	COMPLEJIDAD: Determinar aqu�, justificadamente, la complejidad de la operaci�n cortaPorK

	La complejidad de este ejercicio es lineal (O(n)) ya que hay que buscar el nodo k por el cual hay que cortar.
	**/
	void cortaPorK(unsigned int k, Lista<T>& destino) {
		// A IMPLEMENTAR
		if (k >= _numElems ) return;
		...
		destino._prim = aux->_sig;

		destino._ult = _ult;
		destino._prim->_ant = NULL;
		
		_ult = aux;
		_ult->_sig = NULL;


		// Decimos el numero de elementos.
		destino._numElems = _numElems - k;
		_numElems = k;
	}

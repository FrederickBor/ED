...
public:
	...
	void cambiar_nodo(Nodo *min){
		if (min->_sig == NULL) return;

		// Aplico el cambio
		Nodo* min_sig = min->_sig;
		// Aislo el nodo a cambiar
		if (min != _prim)
			min->_ant->_sig = min_sig;
		else 
			_prim = min_sig;
		min_sig->_ant = min->_ant;

		// Cambio el nodo
		if (min_sig->_sig != NULL)
			min_sig->_sig->_ant = min;
		else
			_ult = min;
		
		min->_ant = min_sig;
		min->_sig = min_sig->_sig;
		min_sig->_sig = min;
	}
    ...
    /* 
	  Complejidad: Determina justificadamente la complejidad de esta operaci�n
		...
		La complejidad de esta operacion es Lineal (O(n)) ya que debo buscar 
		el nodo que se va a mover en toda la lista.
	...
	*/
	void transponer() {
		...
		// Busco la ultima aparicion del menor
		// elemento de la lista
		while (aux != NULL){
			if (aux->_elem <= min->_elem){
				min = aux;
			}

...
public:
   ...
   /**
	 * Esta operacion mueve el nodo a_mover como el siguiente de la referencia
	 * y acomoda los punteros para que la lista siga siendo valida.
	*/
	void mueve_nodo(Nodo* a_mover, Nodo* ref){
		// Aislo el nodo a_mover
		a_mover->_sig->_ant = a_mover->_ant;
		a_mover->_ant->_sig = a_mover->_sig;

		// Muevo el nodo a_mover como el siguiente de la referencia
		// en caso de que sea el ultimo entonces muevo el 
		// puntero del ultimo nodo
		a_mover->_sig = ref->_sig;
		a_mover->_ant = ref;
		ref->_sig = a_mover;

		if (a_mover->_sig == NULL) _ult = a_mover;
		else a_mover->_sig->_ant = a_mover;
	}

	/** 
	 * Funcion estremecer.
	 * Tiene coste O(n/2) donde n es el numero de elementos en el vector;
	 * 
	*/ 
	void estremece(){
		// Si no tiene al menos 3 elementos entonces 
		// no hacemos nada.
		if (_numElems <= 2) return;

		Nodo* a_mover = _prim;
		Nodo* ref = _ult;

		while (a_mover != ref && a_mover->_sig != NULL && a_mover->_sig != ref ){
			mueve_nodo(a_mover->_sig, ref);
			a_mover = a_mover->_sig;
		}
